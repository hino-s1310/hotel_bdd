...
def step_when(plans_page:PlansPage, reserve_page:ReservePage, plan_name):
...
def step_then(confirm_page:ConfirmPage, plan_name):
    expect(confirm_page.plans).to_have_text(plan_name)
...
def step_then(confirm_page:ConfirmPage, additional_plan):
    expect(confirm_page.plan_name).to_have_text(additional_plan)

@then(parsers.parse('お名前が「{name}」であることを確認する'))
def step_then(confirm_page:ConfirmPage, name):
    expect(confirm_page.username).to_have_text(name)
...
def step_then(confirm_page:ConfirmPage, plans_page:PlansPage):
    # ページハンドリング
    with confirm_page.page.context.expect_page() as new_page_info:
        confirm_page.click_confirm(),
    new_page = new_page_info.value
    confirm_page.page.on("new_page", handle_page)

    #PlansPageインスタンス生成
    plans_page =  PlansPage(new_page)
